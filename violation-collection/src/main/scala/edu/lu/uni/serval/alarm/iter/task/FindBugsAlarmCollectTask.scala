...
class FindBugsAlarmCollectTask extends CommitTask with LazyLogging {
  ...
  def setStatLogger(p: StringBuffer) = {
    this.statLogger = p;
  }

  def setRepoDir(p: String) = {
    this.repoDir = p;
  }

  def setMavenTempRepoDir(p: String) = {
    this.mvnRepo = p;
  }

  def setReportsRoot(p: String) = {
    reportsRoot = p;
  }

  def setReleaseName(p: String) = {
    releaseName = p;
  }

  def setProjectName(p: String) = {
    projectName = p;
  }

  def setFilters(p: Buffer[String]) = {
    filters = p;
  }

  def setLogDir(p: File) = {
    logDir = p;
  }
